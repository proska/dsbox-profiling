...
def __tableGen(data):
    ...
    for name in data:
        ...
        # for numbers w/ nunique > 2 (when there are more then 1 such columns)
            ...
            table = table.append({
                                     'col_name': name, 'nunique': col.nunique(),
                                     'nunique_ratio': round(float(col.nunique()), 4) / col.count(),
                                     'H': lvl[0], 'M': lvl[1], 'L': lvl[2],
                                     'ratio_H': lvl_ratio[0], 'ratio_M': lvl_ratio[1],
                                     'ratio_L': lvl_ratio[2],
                                     'dropMean': round(np.mean(p), 4),
                                     'dropMedian': round(np.median(p), 4),
                                     'dropMax': round(np.max(p), 4), 'dropMin': round(np.min(p), 4),
                                     'dtype': col.dtype, '95%in10': in10
                                 }, ignore_index=True)
...
def category_detect(data):
    ...
    for index, row in table.iterrows():
        res = __column_detect(row['dtype'], row['nunique'], row['nunique_ratio'], row['dropMax'],
                              row['H'], row['M'])
